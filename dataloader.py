...
def get_dataset(
  ...
  # For BERT tokenizer, use CLS as BOS and SEP as EOS
  # BERT doesn't have traditional BOS/EOS tokens, but uses CLS and SEP instead
  if hasattr(tokenizer, 'cls_token') and tokenizer.cls_token is not None:
    BOS = tokenizer.cls_token_id
  else:
    BOS = tokenizer.encode(tokenizer.bos_token, add_special_tokens=False)[0] if tokenizer.bos_token else None
    
  if hasattr(tokenizer, 'sep_token') and tokenizer.sep_token is not None:
    EOS = tokenizer.sep_token_id  
  else:
    EOS = tokenizer.encode(tokenizer.eos_token, add_special_tokens=False)[0] if tokenizer.eos_token else None
